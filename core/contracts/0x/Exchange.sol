...
contract Exchange is SafeMath {
    ...
    event LogRounding(uint indexed salt, uint8 indexed errorId);

    event LogAllowance(uint indexed salt, uint8 indexed errorId);
    ...
    */
          ...
          returns (uint filledTakerTokenAmount)
    {
        ...
        if (isRoundingError(filledTakerTokenAmount, order.takerTokenAmount, order.makerTokenAmount)) {
            LogRounding(1111, uint8(Errors.ROUNDING_ERROR_TOO_LARGE));
