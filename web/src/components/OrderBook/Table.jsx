  ...
  <Table className="Table">
      ...
      </div>}
    {
      orders.length > 0 ? orders.map((order, i) => (
        <div key={order.id} style={{ position: 'relative', marginTop: i === 0 && type ==='sell' ? 'auto' : 0 }} className="Table-row" onClick={() => fillOrder(order)}>
          <div className="Table-row-item" data-header="Header1">
            <Colored color={type === 'sell' ? 'red' : 'green'}>{order.price}</Colored>
          </div>
      ...
      :
      <div style={{ margin: '10% auto' }}>No orders</div>
    }
  </Table>);
