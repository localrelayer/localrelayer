...
(async () => {
  ...
  const erc20AssetProxyResult = await contractWrappers.exchange.getAssetProxyBySignatureAsync(
    erc20ProxyId,
  );
  const erc721AssetProxyResult = await contractWrappers.exchange.getAssetProxyBySignatureAsync(
    erc721ProxyId,
  );
  ...
  const erc20ProxyAuthorisedAddresses = await contractWrappers.erc20Proxy
  ...
  const erc721ProxyAuthorisedAddresses = await contractWrappers.erc721Proxy
    .getAuthorizedAddressesAsync();
